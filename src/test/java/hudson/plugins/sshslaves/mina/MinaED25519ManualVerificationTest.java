...
/**
 * Tests SSH connection with ED25519 manual host key verification.
 * Ported from CloudBees ED25519SSHLauncherHostManualVerificationTest.
 */
@Timeout(value = 10, unit = TimeUnit.MINUTES)
@WithJenkins
@Testcontainers(disabledWithoutDocker = true)
class MinaED25519ManualVerificationTest {
    ...
    void sshConnectWithManualKeyVerification() throws Exception {
        ...
        try (GenericContainer<?> sshContainer = MinaSSHContainerFactory.createContainer("ed25519")) {
            sshContainer.start();
            String host = sshContainer.getHost();
            int port = sshContainer.getMappedPort(22);

            // Extract the dynamically generated ED25519 host key from the container
            Container.ExecResult result = sshContainer.execInContainer("cat", "/etc/ssh/ssh_host_ed25519_key.pub");
